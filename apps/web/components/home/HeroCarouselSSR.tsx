...
import type { HeroSlide as CMSHeroSlide } from '@/types/cms';
...
export interface HeroCarouselSSRProps {
  /**
   * CMS slides passed from Server Component.
   * When provided, renders immediately without client fetch.
   */
  cmsSlides: CMSHeroSlide[];
...
/**
 * Default slides used as fallback when CMS returns empty
 */
const defaultSlides: HeroSlide[] = [
  ...
  },
  {
    ...
    ctaLabel: 'Explorer',
    ctaHref: '/categorie/pierres/diamants',
...
/**
 * HeroCarouselSSR - Server-Side Rendering Compatible
 *
 * This component receives hero slide data via props from a Server Component,
 * eliminating the client-side fetch delay and preventing content flash.
 *
 * Features:
 * - SSR-compatible with data passed from Server Component
 ...
 * return <HeroCarouselSSR cmsSlides={heroSlides} />;
 * ```
 */
export const HeroCarouselSSR = memo(function HeroCarouselSSR({
  cmsSlides,
  autoPlayInterval = 5000,
  className,
}: HeroCarouselSSRProps) {
  ...
  // Convert CMS slides to local format, or use defaults
  const slides: HeroSlide[] = useMemo(() =>
    cmsSlides.length > 0 ? cmsSlides.map(cmsSlideToLocal) : defaultSlides,
    [cmsSlides]
  );
...
HeroCarouselSSR.displayName = 'HeroCarouselSSR';

export default HeroCarouselSSR;
